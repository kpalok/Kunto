...
uint32_t GetRegisterData(uint8_t xlsb, uint8_t lsb, uint8_t msb){
    uint8_t xlsb_mask = 0b11110000;
    xlsb = (xlsb & xlsb_mask) >> 4;

    uint32_t fraction_msb = msb;
    fraction_msb = fraction_msb << 12;
    uint32_t fraction_lsb = lsb;
    fraction_lsb = fraction_lsb << 4;
    uint32_t fraction_xlsb = xlsb;

    uint32_t registerData = fraction_xlsb | fraction_lsb | fraction_msb;
...
void bmp280_get_data(I2C_Handle *i2c, double *pres, double *temp) {
    ...
    if (I2C_transfer(*i2c, &i2cTransaction)) {
		...
		// 1) GET THE AIR PRESSURE AND TEMPERATURE VALUES FROM rxBuffer BY USING BIT WISE OPERATIONS (AS IN THE EXERCISES)
    		int32_t raw_pressure = GetRegisterData(rxBuffer[2], rxBuffer[1], rxBuffer[0]);
			int32_t raw_temperature = GetRegisterData(rxBuffer[5], rxBuffer[4], rxBuffer[3]);
		...
		// 3) USE FUNCTION bmp280_convert_pres() TO CONVERT RAW PRESSURE VALUE TO PASCALS
			*temp = bmp280_convert_temp(raw_temperature);
			*pres = bmp280_convert_pres(raw_pressure);
